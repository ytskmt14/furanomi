...
- **ステータス**: 🚧 進行中
- **OpenSpec Change**: 不要
- **説明**: 店舗検索APIのレスポンス時間が約5秒かかっている問題を解決する。Waiting for server responseが4.85秒と非常に長く、UXに大きな影響を与えている。

#### 現状の問題
- Waiting for server response: **4.85秒**（最大のボトルネック）
- Content Download: 850ms
- Total: 約**5.7秒**
- データベースクエリが遅い（複数JOIN + 位置情報計算）
- 全店舗に対して距離計算を2回実行（SQL + JavaScript）
- インデックスが不足している可能性
...
2. **クエリ最適化**
   - 距離計算を1回に（SELECT句で実行）
   - 不要なフィールド削除（shop_manager情報、created_at等）
   - JOINの最適化
