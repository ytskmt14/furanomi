...
- **説明**: 利用者がログイン・登録できる認証機能を実装。メール認証をサポート（LINE認証はPhase 3以降に延期）。予約機能の前提条件。
- **実装内容**:
  - **メール認証**:
...
- **データベース**:
  - `users`テーブル作成（id, email, password_hash, name, phone, reset_token, reset_token_expires_at, last_login_at, created_at, updated_at）
  - セッション管理
- **受け入れ基準**:
  - [x] メールアドレスでユーザー登録ができる
  - [x] メールアドレス・パスワードでログインができる
  - [x] パスワードのハッシュ化が実装されている
  - [x] セッション管理が機能する
  - [x] パスワードリセットトークンの生成ができる（メール送信は未実装）
  - [x] プロフィール情報が保存される
  - [x] 認証が必要なページが保護される
- **関連**: TASK-023（予約機能の前提条件）
- **備考**: LINE認証はPhase 3以降の実装予定
...
- **説明**: 認証済み利用者が店舗を予約できる機能を実装。会員登録した人のみが予約可能。
- **実装内容**:
  - **予約作成**:
    - 認証済みユーザーのみが予約可能
    - 予約者名は会員情報から自動取得
  ...
  - **予約通知**:
    - プッシュ通知での実装（後続のプッシュ通知タスクで対応可）
  - **ヘッダー予約アクセスUI**:
    - ヘッダーに予約通知アイコン（pending予約件数バッジ付き）
    - ユーザーメニュードロップダウン（マイ予約、プロフィール、ログアウト）
- **データベース**:
  - `reservations`テーブル作成（id, user_id, shop_id, party_size, arrival_time_estimate, status, created_at, updated_at）
  - status: 'pending', 'approved', 'rejected', 'cancelled'
- **受け入れ基準**:
  - [x] 認証済み利用者のみが予約できる
  - [x] 予約者名は会員情報から自動取得される
  - [x] 予約情報（人数、何分後）を入力できる
  - [x] 予約の確認・キャンセルができる
  - [x] 店舗管理者が予約一覧を確認できる
  - [x] 店舗管理者が予約を承認・拒否できる
  - [x] 予約ステータスが適切に管理される
  - [x] ヘッダーに予約通知アイコンが表示される（pending件数バッジ付き）
  - [x] ユーザーメニューからマイ予約にアクセスできる
- **前提条件**: TASK-022（利用者認証機能）完了後
- **関連**: TASK-022（利用者認証機能）、TASK-013（プッシュ通知）
