...
- **説明**: 利用者がログイン・登録できる認証機能を実装。メール認証とLINE認証の両方をサポート。予約機能の前提条件。
- **実装内容**:
  - **メール認証**:
  ...
  - **共通機能**:
    ...
    - 認証状態の保持
- **データベース**:
  - `users`テーブル作成（id, provider, provider_id, email, password_hash, name, phone, picture_url, created_at, updated_at）
  - セッション管理
- **受け入れ基準**:
  - [ ] メールアドレスでユーザー登録ができる
  - [ ] メールアドレス・パスワードでログインができる
  - [ ] LINEアカウントでログインができる
  - [ ] LINE認証時に自動的にアカウントが作成される
  - [ ] パスワードのハッシュ化が実装されている
  - [ ] セッション管理が機能する
  - [ ] パスワードリセットができる（メール認証のみ）
  - [ ] プロフィール情報が保存される
