...
- **説明**: 認証済み利用者が店舗ごとに通知ON/OFFを設定できる機能を実装
- **実装内容**:
  - 通知設定画面の追加（マイページ内）
  - 店舗ごとの通知ON/OFF切り替え
  - 通知設定の保存（データベース）
  - ユーザーごとの通知設定管理
- **データベース**:
  - `notification_preferences`テーブル作成（id, user_id, shop_id, enabled, created_at, updated_at）
- **受け入れ基準**:
  - [ ] 通知設定画面が実装されている（マイページ内）
  - [ ] 店舗ごとに通知ON/OFFを切り替えられる
  - [ ] 認証済みユーザーのみ通知設定が可能
  - [ ] 設定がデータベースに保存される
  - [ ] 設定に従って通知が送信される（ONになっている店舗のみ）
- **前提条件**: TASK-013（PWA対応）、TASK-022（利用者認証機能）完了後
- **将来拡張**:
  - 通知時間帯の設定
  - 通知する状態の選択（空きあり、混雑中など）
- **関連**: TASK-013（プッシュ通知基本機能）、TASK-022（利用者認証機能）
