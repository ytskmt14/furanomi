...
（Phase 2の主要タスクは完了しました）

### ✅ Phase 2完了タスク

#### TASK-013: PWA対応（オフライン機能、プッシュ通知）✅ **完了**
...
- **受け入れ基準**:
  ...
  - [x] 利用者向けのホーム画面に追加できる
  - [x] 空き状況変更時にプッシュ通知が送信される
  - [x] アイコンファイルが適切に設定されている

#### TASK-025: お気に入り店舗の空き状況プッシュ通知（利用者）✅ **完了**

- **カテゴリ**: Feature (Notifications)
- **優先度**: Phase 2
- **工数見積**: 大(1-2日)
- **ステータス**: ✅ **完了**
- **説明**: 利用者が店舗をお気に入り登録し、その店舗の空き状況が変更された際にプッシュ通知を受け取れるようにする
- **実装内容**:
  - 利用者アプリに「お気に入り」機能（追加/削除/一覧表示）
  - 空き状況変更イベント発火時に、当該店舗をお気に入り登録しているユーザーへ通知送信
  - 通知内容: 店舗名＋現在の空き状況、遷移先: 該当店舗詳細
  - 通知の受信は利用者アプリ全体で有効（Phase 2方針）
- **データベース**:
  - `user_favorites` テーブル（実装済み）
  - `user_push_subscriptions` テーブル（実装済み）
- **受け入れ基準**:
  - [x] 店舗詳細画面からお気に入り登録/解除ができる
  - [x] マイページ等でお気に入り一覧が表示できる
  - [x] 店舗の空き状況が変更されると、お気に入りユーザーにプッシュ通知が届く
  - [x] 通知から該当店舗詳細に遷移できる
  - [x] 通知ON/OFFはブラウザの通知許諾に従い適切に動作
