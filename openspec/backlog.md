...
#### TASK-021: ランディングページUI/UX改善 ✅

- **カテゴリ**: Improvement (UX)
- **優先度**: 🟡 中
- **工数見積**: 小(1-2h)
- **ステータス**: ✅ 完了
- **OpenSpec Change**: 不要（軽微な改善）
- **説明**: TASK-019で実装したLPのUI/UX改善
- **作業内容**:
  - ServiceOverviewセクション削除（Tutorialと重複）
  - Tutorialセクション強化（利用の流れを統合）
  - Shopsセクション削除（参加店舗情報は不要）
  - フッター簡素化（店舗管理者関連リンク削除）
- **受け入れ基準**:
  - コンテンツの重複が解消されている
  - ページ構成がシンプルで分かりやすい
  - フッターが簡潔になっている
- **関連**: TASK-019（利用者向けLP作成）

#### TASK-019: 利用者向けランディングページ（LP）作成
...
- **説明**: 利用者向けプロモーション用ランディングページ（`/lp`）を作成し、サービス概要、主な機能、価値提案を明確に伝える
- **作業スコープ**:
  - [ ] デザインコンセプトの検討
...
- **工数見積**: 大(1-2日)
- **ステータス**: 🆕 未着手
- **OpenSpec Change**: ✅ **必要** - 新規 `changes/add-pwa-support`
- **説明**: Progressive Web Appとして、オフライン機能とプッシュ通知を実装。利用者向けアプリにPWA機能を追加。
- **実装内容**:
  - Web App Manifest（manifest.json）作成
  - Service Worker実装
  - オフラインキャッシュ戦略（基本機能のオフライン利用）
  - プッシュ通知機能（オプション）
  - ホーム画面追加対応
- **受け入れ基準**:
  - [ ] Service Workerが実装されている
  - [ ] manifest.jsonが設定されている
  - [ ] オフラインで基本機能が使える（店舗一覧・詳細表示）
  - [ ] ホーム画面に追加できる（iOS・Android対応）
  - [ ] プッシュ通知が実装されている（オプション）
  - [ ] アイコンファイルが適切に設定されている
- **対象**:
  - 利用者向けアプリ（`/user/*`）
  - 店舗管理者用アプリ（`/shop-manager/*`）
  - システム管理者用アプリ（`/system-admin/*`）
- **前提条件**: なし
