...
このプロジェクトは**Kiro（Cursor Command - Spec Driven Development）**を使用した仕様駆動開発を行っています。

**新規機能開発の流れ**:
1. `/kiro/spec-init <description>` - 仕様の初期化
2. `/kiro/spec-requirements <feature-name>` - 要件定義（EARS形式）
3. `/kiro/spec-design <feature-name> [-y]` - 技術設計
4. `/kiro/spec-tasks <feature-name> [-y]` - 実装タスク生成
5. `/kiro/spec-impl <feature-name> <task-numbers>` - 実装

**仕様書の場所**:
- `.kiro/specs/` - 仕様書（requirements.md、design.md、tasks.md）
- `.kiro/steering/` - プロジェクトガイドライン（product.md、tech.md、structure.md）

## 開発バックログ

機能追加や改善のタスクは、Gitのコミット履歴やIssueで管理しています。

過去のバックログはGit履歴で確認できます：
```bash
git log --all --grep="TASK-"
```
